...
export default function App() {
  ...
  const loadData = async () => {
    try {     
      const deviceList = await Printer.getDeviceList();
  ...
  const handleConnect = async () => {
    ...
    const printer = await Printer.connectPrinter(selectedDevice);
  ...
  const handlePrintText = async () => {
    ...
    await Printer.printText(execCommands);
  };
  ...
  const handlePrintBill = async () => {
    ...
    execCommands += `${CENTER}${HR3}\n`;
    execCommands += `${BOLD_ON}Billed To:${BOLD_OFF} Test\n`;
    await Printer.printBill(execCommands);
  };
